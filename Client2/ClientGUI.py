...
class GUI:
    def __init__(self, clientMain):
        ...
        self.root2 = Tk()
        self.root2.title("Client2")
        self.root2.geometry("855x500")
        self.root2.resizable(False, False)
        self.LoginButton = Button(self.root2, text="Login", height=1, width=8, command=self.myClick, fg="blue",
                                  bg="pink").grid(padx=5, pady=5, column=0, row=0)
        ...
        self.showOnline = Button(self.root2, text="show online", height=1, width=12, command=self.myClick, fg="blue",
                                 bg="pink").grid(padx=15, pady=5, column=5, row=0)
        self.clear = Button(self.root2, text="Clear", height=1, width=12, command=self.myClick, fg="blue",
                            bg="pink").grid(padx=7, pady=5, column=6, row=0)
        self.someCheck = Checkbutton(self.root2).grid(padx=8, pady=5, column=7, row=0)
        self.showServerFiles = Button(self.root2, text="show server files", height=1, width=13, command=self.myClick,
                                      fg="blue", bg="pink").grid(padx=5, pady=5, column=8, row=0)
        ...
        snd = Button(self.root2, text="Send", height=1, width=11, command=self.sendMessageOut, fg="blue",
                     bg="pink").place(x=746, y=345, height=25)

        SFN = Label(self.root2, text="Server File Name").place(x=5, y=390)
        CFN = Label(self.root2, text="Client2 File Name (save as...)").place(x=265, y=390)
        ...
        prcd = Button(self.root2, text="Proceed", height=1, width=11, command=self.myClick, fg="blue", bg="pink").place(
            x=746, y=410, height=25)
        progras = Progressbar(self.root2, orient=HORIZONTAL, length=100, mode="determinate").place(x=5, y=455,
                                                                                                   width=300)
